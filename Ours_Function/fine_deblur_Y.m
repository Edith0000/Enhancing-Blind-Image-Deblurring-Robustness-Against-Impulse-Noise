function [k, lambda_grad, S] = fine_deblur_Y(blur_B, k, lambda_grad, threshold, opts)
for iter = 1:opts.xk_iter
    %%
    k_prev = k;
    [S,wi] = image_estimate_Y(blur_B, k, lambda_grad,0); % 更新权重W与潜在图像
    %save('./Wi',"wi");
    [k,wk] = psf_fine(blur_B, S, 5, k, threshold,0); % 更新权重W与模糊核
    %save('./Wk',"wk")
    err = k_prev - k;
    if norm(err(:),2)<1e-3
        break;
