function [kernel, interim_latent] = blind_deconv_Y(y, lambda_grad, opts)
...
for s = num_scales:-1:1
  ...
  if s <= 1
      ...
      [ks, lambda_grad, interim_latent] = fine_deblur_Y(ys, ks,lambda_grad, threshold, opts); % 进行精细的去模糊处理
  else
    if opts.isnoisy
        ys = imfilter(ys,fspecial('gaussian',5,1),'same','replicate'); % 如果opts.isnoisy为真，则使用高斯滤波器对输入图像ys进行平滑处理
    end
    [ks, lambda_grad, interim_latent] = coarse_deblur_Y(ys, ks,lambda_grad, threshold, opts); % 进行粗略的去模糊处理
  end
