...
Temporary_path='.\Results\Temporary';
...
for level=level_num:-1:s
    ...
    name=strcat('初始图_',num2str(level),'.png');
    path=fullfile(Temporary_path,name);
    imwrite(image_pyramid{level}, path);

    image_pyramid{level} = imfilter(image_pyramid{level},fspecial('gaussian',5,1),'same','replicate');
    name=strcat('高斯平滑处理后_',num2str(level),'.png');
    path=fullfile(Temporary_path,name);
    imwrite(image_pyramid{level}, path);
    ...
    [Y_b_padding,padsize] = G_padding(image_pyramid{level},k_estimate,1); %扩充边界
    ...
    for iter=1:3
        ...
        name=strcat('骨架图_',num2str(level),'.png');
        path=fullfile(Temporary_path,name);
        imwrite(Y_r_rgtv_cg, path);
        ...
        name=strcat('模糊核_',num2str(level),'.png');
        path=fullfile(Temporary_path,name);
        k_est=k_rescale(k_estimate);
        imwrite(k_est, path);
...
for level=s-1:-1:1
    ...
    name=strcat('index图_',num2str(level),'.png');
    path=fullfile(Temporary_path,name);
    imwrite(index, path);
