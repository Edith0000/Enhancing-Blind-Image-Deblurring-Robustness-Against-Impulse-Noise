function [k, lambda_grad, S] = coarse_deblur_Y(blur_B, k, ...
...
blur_B_w = wrap_boundary_liu(blur_B, opt_fft_size([H W]+size(k)-1)); % 扩充模糊图像边界
blur_B_tmp = blur_B_w(1:H,1:W,:);
Bx = conv2(blur_B_tmp, dx, 'valid');  % 计算梯度
