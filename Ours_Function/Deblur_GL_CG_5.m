function [ x ] = Deblur_GL_CG_5( Y_b,k,W,we,max_iter,data_we)
...
b=imfilter((data_we.*x).*mask,rot90(k,2),'replicate','conv');

if (max(size(k)<25))
    Ax=imfilter((data_we.*imfilter(x,k,'replicate','conv')).*mask,rot90(k,2),'replicate','conv');
else
    Ax=fftconv_BID((data_we.*fftconv_BID(x,k,'same')).*mask,rot90(k,2),'same');
end
...
for i=1:max_iter
     ...
     if (max(size(k)<25))
        Ap=imfilter((data_we.*imfilter(p,k,'replicate','conv')).*mask,rot90(k,2),'replicate','conv');
     else
        Ap=fftconv_BID((data_we.*fftconv_BID(p,k,'same')).*mask,rot90(k,2),'same');
     end
